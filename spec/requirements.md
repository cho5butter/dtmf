...
> 典拠: NTT 600 / 601 形（および同系黒電話）の物理仕様。

- [ ] **数字配置**: 「1」が時計の約2時方向（基準軸から時計回りに60°）、続いて時計回りに `2,3,4,5,6,7,8,9,0` の順で配置される（36°刻み）。「0」は止め金の直前（約300°）
- [ ] **指止め（フィンガーストッパー）**: ディスク外周の **5時方向（約150°）** に固定の突起として描画される。色は `--signal` でアフォーダンスを示す
- [ ] **回転方向**: ユーザーが数字 N をタップ / Then ディスクが**時計回り**に回転し、N の指穴が止め金の位置（150°）に到達した時点で停止する。すなわち回転量は `(150° − digit_angle + 360°) mod 360°`
