...
- [ ] **数字配置**: 10 個の指穴は **30° 刻み**で並び、合計 **270° の円弧**を占める。残る **90° は穴のない隙間**（指止め用）。配置は実機どおり「1」が **4 時方向（時計の約 120°）**、続いて反時計回りに `2, 3, 4, 5, 6, 7, 8, 9, 0` が並び、「0」は **7 時方向（約 210°）**。等間隔分布（旧 36° 刻み・全周一周）にはしない
- [ ] **指止め（フィンガーストッパー）**: ディスク外周の **5 時方向（約 150°）**、すなわち「0」と「1」の間の 90° 隙間の中に固定の突起として描画される。実機では止め金は「1」寄りで「1」から 30°（1 ステップ）・「0」から 60°（2 ステップ）の位置。色は `--signal` でアフォーダンスを示す
- [ ] **回転方向**: ユーザーが数字 N をタップ / Then ディスクが**時計回り**に回転し、N の指穴が止め金の位置（150°）に到達した時点で停止する。回転量 = `N × 30°`（1 が最小 30°、0 が最大 300°）。これは実機のパルス数（N パルス）規格と一致
