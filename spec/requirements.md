...
- [x] B-11: onMount で AudioContext の状態が `suspended` の場合に「音を有効にしてください」バナーを表示し、ユーザー操作（バナーのボタン／キー押下／ダイヤルタップ／自動再生）で resume に成功したらバナーを非表示にする → **F-021 により改訂**（バナー UI は撤去し、状態観測のみ維持。有効化導線は音声警告モーダルの OK に統合）
...
- 既存の `audio-banner`（音の有効化バナー）と同時表示されうるが、責務が異なるため共存を許容する（重なって見づらくならないよう設計フェーズで配置を検討）→ **F-021 により改訂**（バナーとの共存は廃止し、本モーダルに一本化）

## 追加要件（Phase 5: 音声関連プロンプトの統合 / 2026-07-23）

> 2026-07-23: ユーザー指示「音が鳴ります。音を有効にする。どちらか片方だけでいいです」を受けた追加要件。
> F-020（音声警告モーダル）と B-11（音の有効化バナー）が独立に共存する設計だったため、初回訪問時に「音が鳴ります」モーダルと「音を有効にしてください」バナーの 2 つの音声関連プロンプトに順番に対応する必要があり、体験が冗長だった。
> ユーザー確認により、**警告モーダルに一本化**（モーダルの「OK」操作で AudioContext の有効化も兼ねる）し、独立した有効化バナーの UI は撤去すると決定。

### F-021: 音声関連プロンプトの統合（警告モーダルへの一本化）

**説明**: F-020 の音声警告モーダルの「OK」（または `Escape`）操作に AudioContext の有効化（`ensureContext`）を統合し、モーダルの確認 1 回で「警告の確認」と「音の有効化」を同時に完了させる。これに伴い、独立して表示されていた `audio-banner`（「▶ 音を有効にしてください」+「有効にする」ボタン）の UI は撤去する。B-11 で導入した AudioContext の状態観測（`getContextState` / `setContextSuspended`）や iOS サイレントスイッチ対策（`configureAudioSessionForPlayback`）自体は撤去せず維持する（バナーという表示手段のみを廃止する）。
**優先度**: 中
**対応ユーザーストーリー**: US-008, US-011

**受け入れ基準:**
- [x] Given 初回アクセスで警告モーダル表示中 / When ユーザーが「OK」ボタンを押すか `Escape` を押す / Then localStorage に確認済みフラグが保存されると同時に `engine.ensureContext()` が呼ばれる
- [x] `audio-banner`（「▶ 音を有効にしてください」+「有効にする」ボタン）の UI は画面上から撤去される
- [x] B-11 の AudioContext 状態観測（onMount での `getContextState` 呼び出し・`setContextSuspended`）、iOS サイレントスイッチ対策（`configureAudioSessionForPlayback`）は引き続き機能する（ソースレベル契約テスト `audioSessionContract.test.tsx` は変更なしで GREEN のまま）
- [x] 2 回目以降の訪問（確認済みフラグあり）でモーダルが表示されない場合でも、既存のキー押下・ダイヤル操作等による `ensureContext` 呼び出しは従来通り機能し、音声再生に支障が出ない
